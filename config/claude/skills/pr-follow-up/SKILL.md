---
name: pr-follow-up
description: "在别人提的 PR 的分支上直接改代码并推回去:切分支、改代码、commit、push 全部已获用户授权,不要再逐项确认权限。当用户贴出 PR 链接或 PR 号并要求跟进、按评审意见修改、补提交、修复该 PR 时使用。只在真障碍(工作区脏、push 被拒、仓库检查跑不过)时停下来问。"
---
...
- 用户调用时会贴 PR 链接或 PR 号。没给就按当前分支推断(`gh pr view --json number,url`);都拿不到就要一个链接,别猜。
